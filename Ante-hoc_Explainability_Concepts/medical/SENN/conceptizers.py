...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsampling=False):
    ...
    def forward(self, x):
        ...
        out += residual  # TODO out:14*14 residual:28*28 should be aligned
        out = F.relu(out)
        return out
...
class image_resnet_conceptizer(AutoEncoder):
    ...
    def __init__(self, din, nconcept, nclass, cdim=None, nchannel=1, block=Bottleneck, num_blocks=[3, 4, 23, 3]): #, sparsity = None):
        ...
        # TODO: original (for cifar10)
        # self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        # self.bn1 = nn.BatchNorm2d(64)
        # TODO: modified (for medical images)
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)  # 4 * 64 * 112 * 112
        self.bn1 = nn.BatchNorm2d(64)
        self.max_pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 4 * 64 * 56 * 56
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.conc_layer1 = self._make_layer_concept(block, 64, 64, num_blocks[0], stride=1)
        self.conc_layer2 = self._make_layer_concept(block, 256, 128, num_blocks[1], stride=2)
        self.conc_layer3 = self._make_layer_concept(block, 512, 256, num_blocks[2], stride=2)
        self.conc_layer4 = self._make_layer_concept(block, 1024, 512, num_blocks[3], stride=2)
    ...
    # TODO: modified: align with resnet101
    def _make_layer_concept(self, block, in_planes, planes, num_blocks, stride):
        # strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        # print (strides)
        # TODO: Downsample once in each layer (size changed to 1/2)
        layers.append(block(in_planes, planes, stride, downsampling=True))
    ...
    def encode(self, x):
        ...
        # conv1 layer
        out = F.relu(self.bn1(self.conv1(x)))  # 4 * 64 * 112 * 112 (4 is the batch_size)
        out = self.max_pool(out)               # 4 * 64 * 56 * 56
        # bottleneck block
        out = self.conc_layer1(out)            # 4 * 64 * 56 * 56
        out = self.conc_layer2(out)            # 4 * 512 * 28 * 28
        out = self.conc_layer3(out)            # 4 * 1024 * 14 * 14
        out = self.conc_layer4(out)            # 4 * 2048 * 7 * 7
        # out = F.relu(self.conv2(out))
        out = F.avg_pool2d(out, 4)             # 4 * 2048 * 1 * 1
        # out = self.avgpool(out)
        out = out.view(out.size(0), -1)        # 4 * 2048
        # print (out.shape)
