...
def generate_dir_names(dataset, args, make = True):
    if args.h_type == 'input':
        suffix = '{}_Epochs{}_H{}_Th{}_Reg{:0.0e}_LR{}_opt{}'.format(
                    args.theta_reg_type,
                    args.epochs,
                    args.h_type,
                    args.theta_arch,
        ...
        suffix = '{}_Epochs{}_H{}_Th{}_Cpts{}_Reg{:0.0e}_Sp{}_LR{}_opt{}'.format(
                    args.theta_reg_type,
                    args.epochs,
                    args.h_type,
                    args.theta_arch,
...
def concept_grid(args, model, data_loader, cuda=False, top_k=6, layout='vertical', return_fig=False, save_path=None):
    """
        ...
        Args:
            top_k: visualize top k images for each concept

    """

    print('Warning: make sure data_loader passed to this function does not shuffle data!!')
    num_concepts = model.conceptizer.nconcept

    # generate dir for each concept
    for i in range(num_concepts):
        path = 'test/{}_{}'.format(args.dataset, i+1)
    ...
    top_idxs = top_idxs.squeeze().t()  # shape: 10 * 10
    # top_examples = {}
    top_examples = defaultdict(list)
    # print (top_idxs)
    for i in range(num_concepts):
        for j in top_idxs[i]:
            get_image, _ = data_loader.dataset[j]  # get_image.shape: 3 * 32 * 32
    ...
    for i in range(num_concepts):
        for j in range(top_k):
            ...
            # TODO: modified the path
            plt.imsave('test/' + args.dataset + "_" + str(i + 1) + '/' + str(j) + '.png', top_examples[i][j])
