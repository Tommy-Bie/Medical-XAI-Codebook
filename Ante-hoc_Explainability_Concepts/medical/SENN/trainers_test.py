...
def gaussian_noise(inputs, mean=0, stddev=0.38):
    input = inputs.cpu()
...
class ClassificationTrainer():
    ...
    def train(self, train_loader, val_loader = None, test_loader = None, epochs = 10, save_path = None):
        best_prec1 = 0
...
class GradPenaltyTrainer(ClassificationTrainer):
    ...
    def train_batch(self, inputs, targets):
        ...
        all_losses['auxiliary_prediction'] = aux_loss.data[0]
        if self.learning_h:
            h_loss = self.concept_learning_loss(inputs, all_losses)
