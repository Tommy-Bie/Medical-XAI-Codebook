...
class ClassificationTrainer():
    ...
    #loss_type = 'ce', opt = 'adam', lr = 0.0002, cuda = False, log_interval = 100):
        ...
        if args.nclasses <= 2 and args.objective == 'bce':
            ...
            self.prediction_criterion = F.binary_cross_entropy # NOTE: This does not do sigmoid itself
        ...
        if args.opt == 'adam':
            ...
            # TODO: LR scheduler: learning rate decay after 10 epochs
            lr_lambda = lambda epoch: 1.0 if epoch < 10 else np.math.exp(0.1 * (10 - epoch))
            self.scheduler = LambdaLR(optimizer=self.optimizer, lr_lambda=lr_lambda)
    ...
    def train_epoch(self, epoch, train_loader):
        """
            Does mostly accounting. The actual training is done by the train_batch method.
        """
        ...
        for i, (inputs, targets) in enumerate(train_loader, 0):
            ...
            # self.scheduler.step()
            # print("lr:", self.optimizer.param_groups[0]['lr'])  # print learning rate for each epoch
    ...
    def validate(self, val_loader, fold=None):
        ...
        for i, (inputs, targets) in enumerate(val_loader, 0):
            ...
            # TODO: modified
            prec1 = self.accuracy_1(output.data, targets)
    ...
    # TODO: new add: only calculate the precision@1
    def accuracy_1(self, output, target):
        """
        return acc@1
        """
        batch_size = target.size(0)  # 32
        _, pred = output.topk(1, dim=1, largest=True)
        pred = pred.T
        correct = pred.eq(target)
        num_correct = 0
        for res in correct[0]:
            if res:
                num_correct += 1
...
class GradPenaltyTrainer(ClassificationTrainer):
    ...
    def __init__(self, model, args, typ):
        ...
        self.lambd = args.theta_reg_lambda if ('theta_reg_lambda' in args) else 1e-6  # regularization strength
        self.reconst_criterion = nn.MSELoss()
        self.penalty_type = typ
        self.norm = 2

        if self.cuda:
            self.reconst_criterion = self.reconst_criterion.cuda()
    ...
    def train_batch(self, inputs, targets):
        ...
        #     # Then?? Need to do for, bmm does not do 4D. Maybe through sum approach?
