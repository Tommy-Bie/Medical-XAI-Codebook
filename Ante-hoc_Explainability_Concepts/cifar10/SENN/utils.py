...
def generate_dir_names(dataset, args, make = True):
    if args.h_type == 'input':
        suffix = '{}_Epochs{}_H{}_Th{}_Reg{:0.0e}_LR{}'.format(
                    args.theta_reg_type,
                    args.epochs,
                    args.h_type,
                    args.theta_arch,
        ...
        suffix = '{}_Epochs{}_H{}_Th{}_Cpts{}_Reg{:0.0e}_Sp{}_LR{}'.format(
                    args.theta_reg_type,
                    args.epochs,
                    args.h_type,
                    args.theta_arch,
...
def concept_grid(model, data_loader, cuda=False, top_k=6, layout='vertical', return_fig=False, save_path=None):
    ...
    for i in range(num_concepts):
        for j in range(top_k):
            ...
            plt.imsave('test/cifar_' + str(i + 1) + '/' + str(j) + '.png', top_examples[i][j])  # TODO: modify the path
