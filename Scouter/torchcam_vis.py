...
from dataset.ACRIMA import ACRIMA
from dataset.SkinCon import SkinCon
...
def for_vis(args):
    ...
    # CUB
        ...
        transform = transforms.Compose([transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    # ACRIMA
    elif args.dataset == 'ACRIMA':
        dataset_val = ACRIMA(args, train=False, transform=transform)
        data_loader_val = torch.utils.data.DataLoader(dataset_val, args.batch_size, shuffle=False, num_workers=0, pin_memory=True)
        data = iter(data_loader_val).next()
        image = data["image"][0]
        label = data["label"][0]
        image_orl = Image.fromarray((image.cpu().detach().numpy() * 255).astype(np.uint8).transpose((1, 2, 0)), mode='RGB')
    ...
    # SkinCon
        ...
        dataset_val = SkinCon(args, train=False, transform=transform)
        data_loader_val = torch.utils.data.DataLoader(dataset_val, args.batch_size, shuffle=False, num_workers=0,
                                                      pin_memory=True)
        if args.vis_image is None:
            data = iter(data_loader_val).next()
            image = data["image"][0]
            label = data["label"][0]
        # TODO: add image choice
        else:
            data_dir = args.dataset_dir + "/Fitzpatrick17k/images"
            for image_name in os.listdir(data_dir):
                if image_name == args.vis_image:
                    label = dataset_val.getLabel(image_name)
                    image_name = data_dir + "/" + image_name
                    image = Image.open(image_name)
                    resize_transform = transforms.Compose([transforms.Resize((260, 260)),
                                                     transforms.ToTensor()])
                    image = resize_transform(image)

        image_orl = Image.fromarray((image.cpu().detach().numpy() * 255).astype(np.uint8).transpose((1, 2, 0)),
                                    mode='RGB')
        image = transform(image_orl)
        transform = transforms.Compose([transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
